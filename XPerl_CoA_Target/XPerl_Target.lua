...
local function XPerl_Target_ReadPower(unit, powerType, powerToken)
	if (powerType == 0) then
		return UnitMana(unit) or 0, UnitManaMax(unit) or 0
	end

	return UnitPower(unit, powerType) or 0, UnitPowerMax(unit, powerType) or 0
end

function XPerl_Target_GetAlternatePowerType(unit)
	return XPerl_GetAlternatePowerInfo(unit)
end

function XPerl_Target_GetAlternatePowerValue(unit)
	local powerType, powerToken = XPerl_Target_GetAlternatePowerType(unit)
	if (powerType == nil) then
		return 0
	end

	return XPerl_Target_ReadPower(unit, powerType, powerToken)
end

local function XPerl_Target_CreateAlternateBar(self)
	local f = CreateFrame("StatusBar", self.statsFrame:GetName().."druidBar", self.statsFrame, "XPerlStatusBar")
	f:SetPoint("TOPLEFT", self.statsFrame.manaBar, "BOTTOMLEFT", 0, 0)
	f:SetHeight(10)
	f:SetWidth(self.statsFrame.manaBar:GetWidth())
	self.statsFrame.druidBar = f
...
local function XPerl_Target_UpdateFrameHeight(self)
	local totalHeight = self.nameFrame:GetHeight() + self.statsFrame:GetHeight() + 2
	if (self.portraitFrame and self.portraitFrame:IsShown()) then
		totalHeight = max(totalHeight, self.portraitFrame:GetHeight())
	end
	...
	if (self.highlight) then
		...
		if (self.statsFrame:GetBottom() and self.portraitFrame and self.portraitFrame:IsShown() and self.portraitFrame:GetBottom() and self.statsFrame:GetBottom() < self.portraitFrame:GetBottom()) then
...
local function XPerl_Target_UpdateAlternateMana(self)
	if (not self.conf.mana) then
		if (self.statsFrame.druidBar) then
			self.statsFrame.druidBar:Hide()
			self.statsFrame.druidBar.text:Hide()
			self.statsFrame.druidBar.percent:Hide()
		end
		if (self.statsFrame.alternateBarShown) then
			self.statsFrame.alternateBarShown = nil
			self.statsFrame:SetHeight(XPerl_Target_GetBaseStatsHeight(self))
			XPerl_StatsFrameSetup(self, self.statsFrame.druidBar and {self.statsFrame.druidBar} or nil)
			XPerl_Target_UpdateFrameHeight(self)
		end
		return
	end

	if (not UnitIsPlayer(self.partyid)) then
		if (self.statsFrame.druidBar) then
			self.statsFrame.druidBar.percent:Hide()
			self.statsFrame.druidBar.text:Hide()
			self.statsFrame.druidBar:Hide()
			self.statsFrame.druidBar:SetHeight(1)
		end
		if (self.statsFrame.alternateBarShown) then
			self.statsFrame.alternateBarShown = nil
			self.statsFrame:SetHeight(XPerl_Target_GetBaseStatsHeight(self))
	...
	local alternatePower, alternatePowerToken, alternateColourKey = XPerl_Target_GetAlternatePowerType(self.partyid)
	local currentPower, maxPower = 0, 0
	if (alternatePower) then
		currentPower, maxPower = XPerl_Target_ReadPower(self.partyid, alternatePower, alternatePowerToken)
	end
	local showAlternateBar = alternatePower and maxPower > 0
	local druidBar = self.statsFrame.druidBar

	if (showAlternateBar and not druidBar) then
		druidBar = XPerl_Target_CreateAlternateBar(self)
	end
	...
	local colour = alternateColourKey and conf.colour.bar[alternateColourKey]

	druidBar:SetMinMaxValues(0, maxPower > 0 and maxPower or 1)
	druidBar:SetValue(currentPower)
	druidBar.text:SetFormattedText("%d/%d", currentPower, maxPower > 0 and maxPower or 1)
	if (colour) then
	...
	if (maxPower > 0) then
		druidBar.percent:SetFormattedText(percD, currentPower * 100 / maxPower)
	else
	...
	if (self.statsFrame.alternateBarShown ~= showAlternateBar) then
		...
		self.statsFrame:SetHeight(XPerl_Target_GetBaseStatsHeight(self) + (showAlternateBar and 10 or 0))
		XPerl_StatsFrameSetup(self, {druidBar})
		XPerl_Target_UpdateFrameHeight(self)
	end
end
...
--	elseif ((self.conf.level and self.conf.elite) or targetclassification=="Rare+" or targetclassification=="Rare") then
		if (self.conf.eliteGfx) then
			eliteGfx = true
			self.eliteFrame.tex:SetTexture("Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_Elite")
			...
			elseif (targetclassification == "rare") then
				self.eliteFrame.tex:SetTexture("Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_Rare")
				self.eliteFrame.tex:SetVertexColor(1, 1, 1, 1)
			else
				eliteGfx = nil
			end
...
function XPerl_Target_SetManaType(self)
	...
	if (targetmanamax == 0 or not self.conf.mana) then
		...
		XPerl_Target_UpdateAlternateMana(self)
		return
	end
...
function XPerl_Target_SetMana(self)
	...
	if (p == 0 and targetmanamax > 0) then
