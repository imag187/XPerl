...
local function XPerl_Raid_UpdateMana(self)
	if (rconf.mana) then
		...
		if (rconf.manaPercent and UnitPowerType(partyid) == 0 and not self.pet and manamax > 0) then
...
function XPerl_Raid_Single_OnLoad(self)
	...
	self.edgeFile = "Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_ThinEdge"
	self.edgeSize = 10
	self.edgeInsets = 2
...
function XPerl_Raid_Events:PLAYER_ENTERING_WORLDsmall()
	...
	if (IsInInstance()) then
		LoadAddOn("XPerl_CoA_CustomHighlight")
	end
end
