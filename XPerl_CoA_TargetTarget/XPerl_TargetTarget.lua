...
function XPerl_TargetTarget_UpdateDisplay(self,force)
	...
	if (self.conf.enable and UnitExists(self.parentid) and UnitIsConnected(partyid)) then
		...
		if (self.targetname ~= nil) then
			...
			self.targetaltmana = XPerl_Target_GetAlternatePowerValue and XPerl_Target_GetAlternatePowerValue(partyid) or 0
...
function XPerl_TargetTarget_OnUpdate(self, elapsed)
	...
	local newAltMana = XPerl_Target_GetAlternatePowerValue and XPerl_Target_GetAlternatePowerValue(partyid) or 0
	local newAFK = UnitIsAFK(partyid)

	if ((newHP ~= self.targethp) or (newMana ~= self.targetmana) or (newAltMana ~= self.targetaltmana) or (newAFK ~= self.afk)) then
...
function XPerl_TargetTarget_SetWidth(self)
	...
	self:SetHeight(self.nameFrame:GetHeight() + self.statsFrame:GetHeight() + 2)
