...
local function SetTex(self, num)
	...
	if ((self.GetFrameType or self.GetObjectType)(self) == "Button") then
		if (conf.highlightSelection == 1) then
			self:SetHighlightTexture("Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_Highlight", "ADD")
	...
	elseif ((self.GetFrameType or self.GetObjectType)(self) == "Frame") then
		if (self.tex) then
			self.tex:SetTexture("Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_Highlight", "ADD")
...
function XPerl_OptionActions(which)
	...
	if (conf.showTutorials) then
		if (not IsAddOnLoaded("XPerl_CoA_Tutorial")) then
			EnableAddOn("XPerl_CoA_Tutorial")
			LoadAddOn("XPerl_CoA_Tutorial")
		end
	end
