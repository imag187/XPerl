...
do
	...
	function XPerl_RegisterSMBarTextures()
		...
		shortlist = {
			{"Perl v2", "Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_StatusBar"},
		}
		for i = 1,9 do
			local name = i == 2 and "BantoBar" or "X-Perl "..i
			tinsert(shortlist, {name, "Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_StatusBar"..(i + 1)})
		end
		...
		if (media) then
			...
			media:Register("border", "X-Perl Thin", "Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_ThinEdge")
		end
	end
...
function XPerl_MinimapMenu_Initialize(self, level)
	...
	if (IsAddOnLoaded("XPerl_CoA_RaidHelper")) then
	...
	if (IsAddOnLoaded("XPerl_CoA_RaidMonitor")) then
	...
	if (IsAddOnLoaded("XPerl_CoA_RaidAdmin")) then
...
local xpModList = {"XPerl_CoA", "XPerl_CoA_Player", "XPerl_CoA_PlayerBuffs", "XPerl_CoA_PlayerPet", "XPerl_CoA_Target", "XPerl_CoA_TargetTarget", "XPerl_CoA_Party", "XPerl_CoA_PartyPet", "XPerl_CoA_ArcaneBar", "XPerl_HUD", "XPerl_CoA_RaidFrames", "XPerl_CoA_RaidHelper", "XPerl_CoA_RaidAdmin", "XPerl_CoA_RaidMonitor", "XPerl_CoA_RaidPets", "XPerl_GrimReaper"}
...
function XPerl_MinimapButton_Details(tt, ldb)
	...
	if (UpdateAddOnMemoryUsage and IsAltKeyDown()) then
		...
		local usedKB = GetAddOnMemoryUsage("XPerl_CoA_Options")
		if ((usedKB or 0) > 0) then
...
local curseEdge = "Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_Curse"
					...
					if (buff and (((mine == 1) and (isMine or isStealable)) or ((mine == 2) and not (isMine or isStealable)))) then
						...
						if (isStealable) then		--  and UnitCanAttack("player", partyid)
							if (not button.steal) then
								...
								button.steal.tex:SetTexture("Interface\\AddOns\\XPerl_CoA\\Images\\StealMe")
...
function XPerl_RegisterScalableFrame(self, anchorFrame, minScale, maxScale, resizeTop, resizable, scalable)
	...
	if (not self.corner) then
		...
		self.corner.tex:SetTexture("Interface\\AddOns\\XPerl_CoA\\Images\\XPerl_Elements")
