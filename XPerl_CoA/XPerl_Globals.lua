...
do
	...
	local name,_,_,enabled,loadable = GetAddOnInfo("XPerl_CoA_Party")
	if (enabled) then
...
function XPerl_LoadOptions()
	if (not IsAddOnLoaded("XPerl_CoA_Options")) then
		EnableAddOn("XPerl_CoA_Options")
		local ok, reason = LoadAddOn("XPerl_CoA_Options")
...
function XPerl_ValidateSettings()
	if (conf and conf.coaDefaultsVersion ~= 1) then
		conf.player = conf.player or {}
		conf.player.showRunes = 1
		conf.player.dockRunes = 1
		conf.coaDefaultsVersion = 1
	end
